<template>
  <div class="min-h-screen bg-gray-100">
        ...
        <div class="flex flex-col md:flex-row gap-8">
          <div class="flex-1 flex flex-col items-center">
            <div class="grid grid-cols-3 gap-2 mb-4">
              <img v-for="(img, idx) in product.images" :key="idx" :src="`/storage/${img.image_path}`" class="w-24 h-24 object-cover rounded cursor-pointer border-2 border-transparent hover:border-blue-500" @click="openLightbox(idx)" />
            </div>
            <img :src="`/storage/${product.images[lightboxIndex]?.image_path || product.images[0]?.image_path}`" class="w-64 h-64 object-cover rounded shadow mb-4" v-if="product.images && product.images.length" />
    ...
    <div v-if="showLightbox" class="fixed inset-0 bg-black bg-opacity-80 flex items-center justify-center z-50">
      <img :src="`/storage/${product.images[lightboxIndex]?.image_path}`" class="w-2/3 max-w-2xl rounded shadow-lg" />
