          ...
          <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="bg-white divide-y divide-gray-200">
              <tr v-for="category in categories" :key="category.id">
                ...
                <td class="px-6 py-4 whitespace-nowrap">{{ category.is_active ? 'Yes' : 'No' }}</td>
...
export default {
  ...
  methods: {
    ...
    saveCategory() {
      if (this.isEdit) {
        router.put(`/admin/categories/${this.editCategoryId}`, this.form, {
          onSuccess: () => this.resetForm()
        });
      } else {
        router.post('/admin/categories', this.form, {
          onSuccess: () => this.resetForm()
        });
      }
    },
